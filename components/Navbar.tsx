...
const Navbar = () =>{
    ...
    return (
        <div className={"w-full overflow-x-hidden text-white/70 pt-6"}>
            <div className={"hidden md:flex items-center px-4 py-2 mx-auto max-w-[400px]"}>
                <ul className={"flex flex-row p-4 space-x-8"}>
                    {navlinks.map((link, index) => (
                        <li key={index}>
                            <Link href={link.path}>
                              <p className={" text-lg font-bold cursor-pointer"}>{link.title}</p>
                            </Link>
                        </li>
                    ...
                    <li>
                        <a href={"#contact"} className={"group"}>
                            <h1 className={"text-lg font-bold text-white opacity-70 cursor-pointer"}>Contact Me</h1>
